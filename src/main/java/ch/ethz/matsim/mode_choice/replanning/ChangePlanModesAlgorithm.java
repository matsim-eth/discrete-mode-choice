...
public class ChangePlanModesAlgorithm implements PlanAlgorithm{
	...
	public ChangePlanModesAlgorithm(final Random rng, ModeChoiceModel modeChoiceModel,
			Network network) {
		this.rng = rng;
	...
	public void run(Plan plan) {
		List<String> selectedModes = modeChoiceModel.chooseModes(plan.getPerson(), plan);		
		...
		for (int i = 0; i < selectedModes.size(); i++) {
			setLegMode(plan.getPerson(), legs.get(i), activities.get(i).getLinkId(), activities.get(i + 1).getLinkId());
		}
	}
