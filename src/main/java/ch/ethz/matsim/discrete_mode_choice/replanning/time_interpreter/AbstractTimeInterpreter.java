...
public abstract class AbstractTimeInterpreter implements TimeInterpreter {
	...
	protected void verify() {
		if (Time.isUndefinedTime(currentTime)) {
			throw new IllegalStateException("Illegal current time while interpreting times");
		}

		if (Time.isUndefinedTime(previousTime)) {
			throw new IllegalStateException("Illegal previous time while interpreting times");
		}
	...
	public void addLeg(Leg leg) {
		if (leg.getRoute() == null) {
			advance(currentTime + leg.getTravelTime());
		} else {
			advance(currentTime + leg.getRoute().getTravelTime());
		}
	}
